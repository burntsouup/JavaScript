...
- **Promise functions**
    ...
    :bulb: addresses concerns with callbacks such as *callback hell* (nested callbacks) and *error handling*
    ...
    2. **settled**:
        ...
        a. **fulfilled** - async operation succeeded, returning a (resolve) *value*

        b. **rejected** - async operation failed, returning an (reject) *error*/*reason*
    ...
    - once the Promise settles, it (asynchronously) invokes any further handler functions by using the `then()` and `catch()` methods:
        ...
        - `then()` - invoked when a promise is either fulfilled or rejected. It takes 2 callback handler functions as parameters, the first executes if the promise is fulfilled, returning a value, and the second executes if the promise is rejected, returning an error

            ```JavaScript
            .then(function(value){...}, function(error){...})
            ```

        - `catch()` - invoked when a promise is either rejected or some error has occurred (error handler). It takes 1 callback handler function as a parameter
    ...
    - Example - as a kid, your mom *promises* you that she will buy you a new phone in the future. At this time, you don't 100% know if you will get a phone or not. Your mom will either keep the promise or not based on how she feels

        - this promise has 3 states:

            1. *pending* - you don't know if you will get a phone

            2. *settled*:

                a. *fulfilled* - your mom buys you a phone; the promise is *resolved*

                b. *rejected* - your mom doesn't buy you a phone; the promise is *rejected*

        - the promise *settles* once your mom will eventually decides between:

            1. fulfilling her promise and *then* giving you a phone

            2. rejecting her promise and giving (*catch*) you a *reason* (error) why
    ...
    - **Async** & **Await**:
        ...
        :bulb: `async`/`await` isn't adding any new functionality, it's just an easier way to write `Promises` (*syntactic sugar*)
